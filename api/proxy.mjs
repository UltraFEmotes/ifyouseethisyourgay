...
//
// Routed here via vercel.json rewrites — a single-file catchall works around
// Vercel's flakiness with `[...slug]` filename-based catch-alls.
...
export default async function handler(req, res) {
  try {
    ...
    // The rewrite in vercel.json passes the original URL through as `x-forwarded-uri`
    // (via `has` header) OR we can just parse req.url directly. Vercel keeps the
    // original path in req.url after rewrites.
    const raw = req.url || "/";
    ...
    // Vercel may add rewrite-metadata params — strip anything that looks like it.
    params.delete("path");
